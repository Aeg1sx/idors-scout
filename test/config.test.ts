...
test("loadConfig filters non-GET manual targets while safe mode is enabled", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "idor-config-safe-targets-test-"));
  try {
    const configPath = path.join(tempDir, "config.json");
    const rawConfig = {
      baseUrl: "https://api.example.com",
    ...
    const { config } = await loadConfig(configPath);
    assert.deepEqual(config.scan.methods, ["GET"]);
    assert.deepEqual(config.targets.map((target) => target.method), ["GET"]);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
